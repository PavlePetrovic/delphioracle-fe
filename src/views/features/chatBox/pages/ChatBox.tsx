...
import { askQuestion, getUserData } from "../reducer/chatBox.actions";

import { coolPunchLines } from "../chatData";
import { messageType, questionsCategoriesType } from "../chatBoxTypes";
import { updateCredits } from "../../profile/reducer/profile.actions";
...
const ChatBox = ({ isPublic }: { isPublic?: boolean }) => {
  ...
  const sendQuestion = (messageContent: string, credits: number) => {
    ...
    if (messageContent) {
      if (!isPublic && !chatData.value?.account_info?.credits) {
  ...
  useEffect(() => {
    if (
      ...
      dispatch(setCentralizedLoading(true));
      setTimeout(() => {
        dispatch(getUserData({ user_id: tmpUserId }));
      }, 20000);
      ...
      >
        ...
        {loading && (
          <div className="fade-in-message-animation flex flex-col gap-2">
            <div className="bouncing-loader mr-auto flex animate-pulse items-center rounded-xl bg-[#a1aac5e7] px-2.5 pb-2 pt-3 text-sm ">
              <div className="m-[2.5px] h-[4px] w-[4px] rounded-full bg-[#0D101A] opacity-100"></div>
              <div className="m-[2.5px] h-[4px] w-[4px] rounded-full bg-[#0D101A] opacity-100"></div>
              <div className="m-[2.5px] h-[4px] w-[4px] rounded-full bg-[#0D101A] opacity-100"></div>
            </div>
          </div>
    ...
    <>
      ...
      {true ? (
        ...
        >
          ...
          />
          <button
            className="just flex items-center justify-center gap-2 rounded-full bg-transparent-gray px-3 py-[5px] font-light w888:gap-1.5 w888:px-2 w888:py-[7.5px]"
            type="submit"
            disabled={loading}
          >
        ...
        <CreatePortal>
          <BoxModal>
            <div className="flex h-full w-full flex-col items-center justify-center gap-8 p-10">
              ...
              {!isObjEmpty(chatData.value?.account_info?.user_info) && (
                <p className="w-fit text-center text-base font-medium text-[#ebebeb] drop-shadow-lg">
                  We are expecting you,{" "}
                  {chatData.value?.account_info.user_info.name}!
                </p>
              )}
              <button
...
// Animated loading three dots
// <div className="flex flex-col gap-2 fade-in-message-animation">
//   <div className="bouncing-loader flex items-center rounded-xl bg-[#c2cae4e7] animate-pulse mr-auto pb-2 pt-3 px-2.5 text-sm ">
//     <div className="w-[4px] h-[4px] m-[2.5px] rounded-full bg-[#0D101A] opacity-100"></div>
//     <div className="w-[4px] h-[4px] m-[2.5px] rounded-full bg-[#0D101A] opacity-100"></div>
//     <div className="w-[4px] h-[4px] m-[2.5px] rounded-full bg-[#0D101A] opacity-100"></div>
//   </div>
// </div>
